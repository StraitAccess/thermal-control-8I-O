...
void SerialReceive()
{
  ...
  {
    therm_set[3]=double(foo.asFloat[0]);
    //therm_act[0]=double(foo.asFloat[1]);       // * the user has the ability to send the 
                                          //   value of "Input"  in most cases (as 
                                          //   in this one) this is not needed.
    if(Auto_Man==0)                       // * only change the output if we are in 
    {                                     //   manual mode.  otherwise we'll get an
      therm_pwm[3]=double(foo.asFloat[2]);      //   output blip, then the controller will 
    }                                     //   overwrite.
...
{
  Serial.print("PID ");
  Serial.print(therm_set[3]);   
  Serial.print(" ");
  Serial.print(therm_act[6]);   
  Serial.print(" ");
  Serial.print(therm_pwm[3]);   
  Serial.print(" ");
  Serial.print(therm4_PID.GetKp());   
  Serial.print(" ");
  Serial.print(therm4_PID.GetKi());   
  Serial.print(" ");
  Serial.print(therm4_PID.GetKd());   
  Serial.print(" ");
  if(therm4_PID.GetMode()==AUTOMATIC) Serial.print("Automatic");
  else Serial.print("Manual");  
  Serial.print(" ");
  if(therm4_PID.GetDirection()==DIRECT) Serial.println("Direct");
